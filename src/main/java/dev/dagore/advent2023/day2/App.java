...
public class App {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      int powerOfTheSets = 0;

      while (scanner.hasNextLine()) {
        int powerOfSet = getPowerOfMinimumSetOfCubesNeeded(scanner.nextLine());
        powerOfTheSets += powerOfSet;
      }

      System.out.println("The sum of the power of the sets is equal to: " + powerOfTheSets);
  ...
  private static int getPowerOfMinimumSetOfCubesNeeded(String line) {
    int minNumberOfRedCubes = 1;
    int minNumberOfGreenCubes = 1;
    int minNumberOfBlueCubes = 1;

    StringTokenizer tokens = new StringTokenizer(line, ":,; ");
    // Take "Game" and id out.
    tokens.nextToken();
    tokens.nextToken();
    ...
    while (tokens.hasMoreTokens()) {
      ...
      if (previousToken.matches("\\d+")) {
        ...
        switch (currentToken) {
          case "red":
            minNumberOfRedCubes = Math.max(n, minNumberOfRedCubes);
            break;
          case "green":
            minNumberOfGreenCubes = Math.max(n, minNumberOfGreenCubes);
            break;
          case "blue":
            minNumberOfBlueCubes = Math.max(n, minNumberOfBlueCubes);
            break;
        }
      }
    ...
    return minNumberOfRedCubes * minNumberOfGreenCubes * minNumberOfBlueCubes;
  }
}
